...
void timer_init() {
    ...
    // trigger interrupt whenever the clock addup to the interval
...
u64 timer_get_ticks() {
    u32 hi = REGS_TIMER->counter_high;
    u32 lo = REGS_TIMER->counter_lo;

    // double check if high value didn't change after setting it...
    if (hi != REGS_TIMER->counter_high) {
        hi = REGS_TIMER->counter_high;
